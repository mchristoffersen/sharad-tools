...
#
# Written by Stefano Nerozzi, last edit 28-Jun-2022
#
# To run this script:
# ./findSurface.bash -a <study area name> -w <window size> -s <smoothing width> -l <profile list> -r <PDS .IMG dir path> -n <nav file dir path> -f <seisware_flag> -c <number of cpus>
#
...
mkdir -p ${study_area}
if [[ ${seis_flag} == "y" ]]; then
  mkdir -p ${study_area}/seisware_surface
fi
while read profile; do
  ...
  if [[ -z "${rad_path}" ]]; then
    ...
    echo "Warning: Cluttersim nav ${profile} is missing" | tee -a ${study_area}/log
  else
...
# Use GNU Parallel to unleash the Python
