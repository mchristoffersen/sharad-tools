...
def main(EDRName, auxName, lblName, chirp = 'synth', presumFac = None):
    ...
    if chirp =='calib':
        ...
        chirpWin[0:2048] = np.conj(refChirp)
        ...
        for _i in range(records):
            ...
            sciFFT = np.fft.fft(sciPad) / len(sciPad)
    ...
    data = 10 * np.log10(np.power(np.abs(np.real(EDRData))/np.amax(np.abs(np.real(EDRData)))),2)
    print(data)
    pow_out = np.power(np.real(EDRData), 2) / 50
